...
static int oled_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /* c. 分配显存 */
    // oled_fb_info->screen_base = dmam_alloc_coherent(&client->dev, oled_fb_info->fix.smem_len, &phy_addr_fb, GFP_KERNEL);
    oled_fb_info->screen_base = devm_kzalloc(&client->dev, oled_fb_info->fix.smem_len, GFP_KERNEL);
    if (oled_fb_info->screen_base == NULL)
    {
        printk(KERN_ERR "oled_i2c_probe: failed to allocate memory for oled_fb_info->screen_base\n");
        return -ENOMEM;
    }
