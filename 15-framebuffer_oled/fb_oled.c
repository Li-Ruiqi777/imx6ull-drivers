...
{
    unsigned char byte = *(oled_fb_info->screen_base +
                           y * oled_fb_info->fix.line_length + (x >> 3));
    int           bit  = x & 0x7;
    if (byte & (1 << bit))
        return 1;
    else
...
static int oled_thread(void *data)
{
    ...
    while (1)
    {
        if (oled_fb_info->screen_base == NULL || oled_fb_info == NULL ||
            data_buf == NULL)
        {
            printk("ERROR! \n");
            return -1;
                          ...
                          const struct i2c_device_id *id)
{
    ...
    {
        printk(KERN_ERR
               "oled_i2c_probe: failed to allocate memory for client\n");
        return -ENOMEM;
    }
    ...
    oled_fb_info->fix.smem_len = oled_fb_info->var.xres *
                                 oled_fb_info->var.yres *
                                 oled_fb_info->var.bits_per_pixel /
    ...
    // oled_fb_info->screen_base = devm_kzalloc(&client->dev,
    // oled_fb_info->fix.smem_len, GFP_KERNEL);
    ...
    oled_fb_info->fix.smem_start =
        virt_to_phys(oled_fb_info->screen_base); // fb的物理地址

    oled_fb_info->fix.type   = FB_TYPE_PACKED_PIXELS; // 表示像素类型
    oled_fb_info->fix.visual = FB_VISUAL_MONO10;      // 表示单色屏幕

    oled_fb_info->fix.line_length = oled_fb_info->var.xres *
                                    oled_fb_info->var.bits_per_pixel /
                                    8; // 每行的长度

    // data_buf = devm_kzalloc(&client->dev, oled_fb_info->fix.smem_len,
    // GFP_KERNEL);
    data_buf = kzalloc(oled_fb_info->fix.smem_len, GFP_KERNEL);
    if (data_buf == NULL)
    {
        printk(KERN_ERR
               "oled_i2c_probe: failed to allocate memory for data_buf\n");
        return -ENOMEM;
    }
