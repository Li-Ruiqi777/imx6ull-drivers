...
static int oled_i2c_probe(struct i2c_client *client,
  ...
  oled_fb_info->fix.smem_len = oled_fb_info->var.xres * oled_fb_info->var.yres *
  ...
  oled_fb_info->screen_base =
      devm_kzalloc(&client->dev, oled_fb_info->fix.smem_len, GFP_KERNEL);
...
static int oled_i2c_remove(struct i2c_client *i2c) { return 0; }
