...
static void timer_callback(unsigned long arg)
{
    ...
    if (val == 0)
    {
        input_report_key(key_dev.inputdev,
                         KEY_0,
                         1);
...
void key_io_init(void)
{
    ...
    request_irq(key_dev.keydesc.irq_num,
                key_dev.keydesc.handler,
                IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING,
                key_dev.keydesc.name,
                &key_dev);
    ...
    key_dev.inputdev->evbit[0] = BIT_MASK(EV_KEY) | BIT_MASK(EV_REP); // 设置输入设备支持的事件类型
    input_set_capability(key_dev.inputdev, EV_KEY, KEY_0);            // 设置输入设备支持的按键
