...
static ssize_t oled_write(struct file *filp, const char __user *buf, size_t cnt,
                          loff_t *off) {
...
static int oled_i2c_probe(struct i2c_client *client,
  ...
  oledcdev.device =
      device_create(oledcdev.class, NULL, oledcdev.devid, NULL, oled_NAME);
...
static void __exit oledcdev_exit(void) { i2c_del_driver(&oled_i2c_driver); }
